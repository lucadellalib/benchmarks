...
"""Recipe for training an LSTM-based language model using discrete audio representations.
...
To run this recipe:
> python train_encodec_lm.py hparams/train_encodec_lm.yaml
...
class LanguageModeling(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # Unflatten
        sig = sig.reshape(
            len(sig), self.hparams.num_speakers, -1
        )  # [B, S, T]

        # Stack along batch dimension
        sig = sig.reshape(-1, sig.shape[-1])  # [B * S, T]

        # Extract audio tokens
        lens = lens[:, None].expand(-1, self.hparams.num_speakers).flatten()
        with torch.no_grad():
            self.hparams.codec.to(self.device).eval()
            tokens = self.hparams.codec.encode(sig, lens)[0]
        batch.tokens = tokens, lens

        # Forward encoder
        enc_out = torch.zeros((*tokens.shape[:-1], self.hparams.joint_dim), device=self.device)
        ...
        bos_idx = (
            bos_idx[:, None]
            .expand(len(tokens) // self.hparams.num_speakers, -1, self.hparams.num_codebooks)
            .flatten(end_dim=-2)
        )

        # Forward decoder
        tokens_bos = torch.cat(
            [bos_idx[:, None], tokens[:, :-1]], dim=-2,
        )
        ce_logits, *_ = self.modules.st_decoder(
            enc_out, tokens_bos, lens
        )
        ...
        if (
            ...
            hyps = [
                ...
                for hyp, rel_length in zip(hyps, lens)
            ]
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = self.hparams.ce_loss(
            ...
            tokens.flatten(start_dim=-2),
            length=lens,
        )

        if hyps is not None:
            targets = tokens.reshape(
                -1,
                self.hparams.num_speakers,
                tokens.shape[1],
                self.hparams.num_codebooks,
            ...
            targets = [
                ...
                for target, rel_length in zip(targets, lens)
            ]
...
def dataio_prepare(hparams):
    ...
    takes = [f"src{i}_wav" for i in range(1, hparams["num_speakers"] + 1)]
    provides = ["sig"]

    def audio_pipeline(*src_wavs):
        # Clean signals
        clean_sigs = []
...
if __name__ == "__main__":
    ...
    if hparams["use_pretrained_embeddings"]:
        ...
        for i in range(hparams["num_codebooks"]):
            ...
            hparams["embedding"].weight.data[start_idx: start_idx + hparams["vocab_size"]].copy_(
