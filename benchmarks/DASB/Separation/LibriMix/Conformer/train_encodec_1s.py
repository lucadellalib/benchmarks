...
class Separation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        with torch.no_grad():
            self.hparams.codec_in.to(self.device).eval()
            self.hparams.codec_out.to(self.device).eval()
            in_tokens = self.hparams.codec_in.encode(in_sig, in_lens)[0]
            out_tokens = self.hparams.codec_out.encode(out_sig, out_lens)[0]
        out_tokens = out_tokens.reshape(
            len(batch),
            self.hparams.num_speakers,
            -1,
            self.hparams.num_codebooks_out,
        )  # [B, S, N, K]
        out_tokens = out_tokens.movedim(-3, -1).reshape(
            len(batch),
            -1,
            self.hparams.num_codebooks_out,  # * self.hparams.num_speakers,
        )  # [B, N, K * S]
        batch.in_tokens = in_tokens, in_lens
        batch.out_tokens = out_tokens, out_lens

        # Forward encoder
        ce_logits = self.modules.st_encoder(in_tokens, in_lens)
        ...
        if (
            ...
            min_length = self.hparams.num_codebooks_out
            hyps = [
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        ce_logits = (
            ce_logits[..., None, :]
            .expand(-1, -1, -1, self.hparams.num_speakers, -1)
            .movedim(-2, 0)
            .flatten(end_dim=1)
        )
        out_tokens = (
        ...
        loss = self.hparams.ce_loss(
            ...
            length=out_lens.expand(self.hparams.num_speakers, -1).flatten(),
            reduction="batch",
        ).reshape(self.hparams.num_speakers, -1)
        loss, idxes = loss.min(dim=0)
        loss = loss.mean()

        # Select tokens corresponding to the speaker with minimum loss
        out_tokens = out_tokens[
            idxes * len(batch) + torch.arange(0, len(batch), device=self.device)
        ]
    ...
    def vocode(self):
        ...
        if self.hparams.use_vocos:
            ...
            vocos = Vocos(
                ...
                bandwidth=self.hparams.bandwidth_out,
        ...
        for score in tqdm(
            ...
            with torch.no_grad():
                if self.hparams.use_vocos:
                    ...
                    hyp_sig = vocos(
                        hyp_tokens, torch.ones(1)
                    )[0].flatten()
                    rec_sig = vocos(
                        rec_tokens, torch.ones(1)
                    )[0].flatten()
                else:
                    self.hparams.codec_out.to(self.device).eval()
                    hyp_sig = self.hparams.codec_out.decode(hyp_tokens)[
                        :, 0
                    ].flatten()
                    rec_sig = self.hparams.codec_out.decode(rec_tokens)[
                        :, 0
                    ].flatten()
            ref_sig = rec_sig#self.test_set[self.test_set.data_ids.index(ID)][
                #"out_sig"
            #].to(
            #    self.device
