...
class Separation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            self.hparams.sem.to(self.device).eval()
            _, sems = self.hparams.sem(in_sig)
            diff = tokens.shape[-2] - sems.shape[-1]
            if diff < 0:
                sems = sems[..., :tokens.shape[-2]]
            elif diff > 0:
                sems = torch.nn.functional.pad(sems, [0, diff])
        split_idx = len(tokens) // (self.hparams.num_speakers + 1)
        in_tokens = tokens[:split_idx]
        in_tokens = torch.cat([in_tokens, sems[..., None]], dim=-1)
