...
"""Recipe for training an autoregressive transformer-based speech separation system using
...
class Separation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        in_tokens = tokens[:split_idx]  # [B, N, K]
        pad_mask = length_to_mask(
            in_lens * in_tokens.shape[1], in_tokens.shape[1]
        ).bool()
        in_tokens[~pad_mask] = self.hparams.pad_index
        in_tokens = in_tokens.flatten(start_dim=1)  # [B, N * K]
        ...
        out_tokens = out_tokens.movedim(-3, -1)  # [B, N, K, S]
        out_tokens = out_tokens.flatten(start_dim=1)
        pad_mask = length_to_mask(
            out_lens * out_tokens.shape[1], out_tokens.shape[1]
        ).bool()
        out_tokens[~pad_mask] = self.hparams.eos_index
        batch.in_tokens = in_tokens, in_lens
        batch.out_tokens = out_tokens, out_lens

        # Prepare BOS tokens
        bos_idx = torch.full(
            (len(out_tokens), 1), self.hparams.bos_index, device=self.device
        )
        out_tokens_bos = torch.cat([bos_idx, out_tokens[:, :-1]], dim=-1,)

        # Forward
        embs = self.modules.embedding(in_tokens)
        enc_out, dec_out = self.modules.transformer(
            embs, out_tokens_bos, in_lens, pad_idx=self.hparams.eos_index,
        )
        ce_logits = self.modules.ce_head(dec_out)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # Prepare EOS tokens
        eos_idx = torch.full(
            (len(out_tokens), 1), self.hparams.eos_index, device=self.device
        )
        out_tokens_eos = torch.cat([out_tokens[:, 1:], eos_idx,], dim=-1,)

        # Cross-entropy loss
        loss = self.hparams.ce_loss(
            ce_logits.log_softmax(dim=-1), out_tokens_eos, length=out_lens,
        )

        if hyps is not None:
            targets = out_tokens.flatten(start_dim=1).tolist()
            ...
            targets = [
                ...
                for target, rel_length in zip(targets, out_lens)
            ]
...
if __name__ == "__main__":
    ...
    if hparams["use_pretrained_embeddings"]:
        weight = hparams["codec"].vocabulary.sum(dim=0)
