...
class Separation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        split_idx = len(tokens) // (self.hparams.num_speakers + 1)
        in_tokens = tokens[:split_idx]
        out_tokens = tokens[split_idx:]  # [B * S, N, K]
        out_lens = lens[split_idx:]  # [B * S]
        batch.in_tokens = in_tokens, in_lens
        batch.out_tokens = out_tokens, out_lens

        # Forward encoder
        enc_out = self.modules.st_encoder(in_tokens, in_lens)
        enc_out = (
            enc_out[:, None]
            .expand(-1, self.hparams.num_speakers, -1, -1)
            .reshape(-1, *enc_out.shape[1:],)
        )

        # Prepare BOS index
        bos_idx = torch.arange(
            self.hparams.bos_index,
            self.hparams.bos_index + self.hparams.num_speakers,
            device=tokens.device,
        )
        bos_idx = (
            bos_idx[:, None]
            .expand(len(in_tokens), -1, self.hparams.num_codebooks)
            .flatten(end_dim=-2)
        )

        # Generate BOS tokens without teacher forcing if specified
        if (
            stage == sb.Stage.TRAIN
            and current_epoch > self.hparams.teacher_forcing_until_epoch
        ):
            with torch.no_grad():
                out_tokens = self.hparams.beam_searcher(
                    enc_out,
                    torch.ones(len(enc_out), device=self.device),
                    bos_idx,
                )

        # Forward decoder
        out_tokens_bos = torch.cat(
            [bos_idx[:, None], out_tokens[:, :-1]], dim=-2,
        )
        ce_logits, *_ = self.modules.st_decoder(
            enc_out, out_tokens_bos, out_lens
        )
        ...
        if (
            ...
            hyps = hyps.flatten(start_dim=-3).tolist()
            ...
            hyps = [
                ...
                for hyp, rel_length in zip(hyps, in_lens)
            ]
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        IDs = batch.id
        _, in_lens = batch.in_tokens
        out_tokens, out_lens = batch.out_tokens
        ...
        if hyps is not None:
            targets = out_tokens.reshape(
                -1,
                self.hparams.num_speakers,
                out_tokens.shape[1],
                self.hparams.num_codebooks,
            ).movedim(1, -1)
            targets = targets.flatten(start_dim=-3).tolist()
            ...
            targets = [
                ...
                for target, rel_length in zip(targets, in_lens)
            ]
...
if __name__ == "__main__":
    ...
    if hparams["use_pretrained_embeddings"]:
        ...
        hparams["encoder_embedding"].weight.data[: len(weight)].copy_(weight)
        for i in range(hparams["num_codebooks"]):
            start_idx = i * (hparams["vocab_size"] + hparams["num_codebooks"])
            start_idx_pretrained = i * hparams["vocab_size"]

            hparams["decoder_embedding"].weight.data[start_idx: start_idx + hparams["vocab_size"]].copy_(
                weight[start_idx_pretrained: start_idx_pretrained + hparams["vocab_size"]]
            )

    # Freeze embeddings
    if hparams["freeze_embeddings"]:
        hparams["encoder_embedding"].requires_grad_(False)
        hparams["decoder_embedding"].requires_grad_(False)
