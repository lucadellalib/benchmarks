...
To run this recipe:
> python train_encodec_st_1q.py hparams/train_encodec_st_1q.yaml
...
class Separation(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        in_tokens = torch.cat(
            [
                in_tokens,
                out_tokens[..., : self.hparams.target_codebook]
                .movedim(1, -1)
                .flatten(start_dim=-2),
            ],
            dim=-1,
        ...
        out_tokens = out_tokens[
            ..., self.hparams.target_codebook : self.hparams.target_codebook + 1
        ]

        out_tokens = out_tokens.movedim(-3, -1).reshape(
            len(batch), -1, self.hparams.num_speakers,
        )  # [B, N, S]
        ...
        out_tokens_bos = torch.cat(
            [
                torch.full(
                    (len(out_tokens), 1, self.hparams.num_speakers,),
        ...
        if (
            ...
            min_length = self.hparams.num_speakers
            hyps = [
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if not self.hparams.use_pit:
            ...
            num_heads = self.hparams.num_speakers
            perm_out_tokens = (
                (
    ...
    def vocode(self):
        ...
        for score in tqdm(
            ...
            hyp_tokens = (
                ...
                .reshape(-1, 1, self.hparams.num_speakers)
                .movedim(-1, 0)
            )
            rec_tokens = (
