...
"""Recipe for training a speech enhancement system using discrete audio representations.
...
class Enhancement(sb.Brain):
    ...
    def toks_to_sig(self, toks):
        ...
        sig = self.hparams.codec.tokens_to_sig(toks, **kwargs)  # [B, T]
        return sig

    def compute_forward(self, batch, stage):
        """Forward pass."""
        batch = batch.to(self.device)
        in_toks, in_lens = batch.in_toks  # [B, N, K]
        out_toks, out_lens = batch.out_toks  # [B, N, K]
...
if __name__ == "__main__":
    ...
    from common import dataio_prepare
